  ...
  <v-container fluid fill-height>
    <v-hover v-slot:default="{ hover }" open-delay="200">
      <v-card :elevation="hover ? 24 : 12">
          ...
          <v-list-item>
            ...
            <v-list-item-content>
              <v-list-item-title>
                ...
                <h5 class="text-h7">{{ item.motoModel.name }}</h5>
              </v-list-item-title>

              <v-list-item-subtitle>
                <!-- Выводим цену товару -->
                <b>{{ item.price }} Золота</b>
              </v-list-item-subtitle>
            </v-list-item-content>
            <v-list-item-action>
        ...
        <v-card-title>
          <h5 class="text-h6">Дата события: {{ item.issue }}</h5>
        </v-card-title>

        <v-card-subtitle>
          <h5 class="text-h7">Место проведения: {{ item.mileage }}</h5>
        </v-card-subtitle>
        ...
        <v-expand-transition>
          <div v-show="show">
            ...
            <v-card-text>
              <p class="text-body-1">Условия: {{ item.characteristic }}</p>
            </v-card-text>
          </div>
        </v-expand-transition>
      </v-card>
